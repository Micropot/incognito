...
class DoccanoStrategy(Strategy):
    def annotate(self, text, coordinate: Dict[List[Tuple], str]):
        ...
        return json.dumps({"text": text, "label": label_data}, ensure_ascii=False)
...
class UimaCasStrategy(Strategy):
    ...
    def annotate(
        self, text: str, coordinate: Dict[Tuple[Tuple[int, int], ...], str]
    ) -> Cas:
        """
        ...
        if not typesystem.contains_type(self.type_name):
            NamedEntityType = typesystem.create_type(
                self.type_name, supertypeName="uima.tcas.Annotation"
            )
            typesystem.create_feature(NamedEntityType, "label", "uima.cas.String")
